...
#
# Usage:
#   From repo root:  frontend/.ci/run-tests.sh
#   From frontend:   ./.ci/run-tests.sh
#
# Customize: Edit frontend/.ci/tests.yml to add/remove or change test commands.
...
# Prefer Node (always in frontend); fallback to Ruby (on GitHub runners)
extract_commands() {
